...
- In-context tracking: do not re-read task from mnem between steps; track
  step completion in context instead
- Checkpoint: write updated progress to Task node after each step (Step 4)
- Install suggestion: if Step 2 triggers a skill install via `find-skills`,
  checkpoint current progress to mnem first so state is preserved if the
  user leaves to install
