...
- No phase-specific conventions; apply agent_id and activation conventions
  from SKILL.md

## Input

- The active `MakeMe:Task` node (goal, scope) — for matching a template
- Available template files in `[skill-dir]/templates/`
...
1. **Read the template file** from `[skill-dir]/templates/[template-name].md`,
   where `[skill-dir]` is the directory containing the Make-Me `SKILL.md`
...
4. **If the user agrees**, create a new template file in `[skill-dir]/templates/`
...
Templates are markdown files in `[skill-dir]/templates/`. To edit:
