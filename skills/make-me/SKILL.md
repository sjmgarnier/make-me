...
Make-Me is designed exclusively for Claude Code / Claude Desktop. The conventions
below override any platform-agnostic defaults and apply to all sub-skills.

### Agent Identity

Always set `agent_id = "claude-desktop"` on all mnem write calls. This value
applies to both Claude Code and Claude Desktop, consistent with the global
...
instructions — they are not registered in the Claude Code plugin system and
cannot be invoked via the Skill tool.

`[skill-dir]` is the directory containing this `SKILL.md` file. Claude Code
injects the skill directory path at session start; use it to construct absolute
paths for Read tool calls.

Canonical sub-skill paths:
- `[skill-dir]/sub-skills/mm-orchestrator.md`
- `[skill-dir]/sub-skills/mm-understand.md`
- `[skill-dir]/sub-skills/mm-plan.md`
- `[skill-dir]/sub-skills/mm-research.md`
- `[skill-dir]/sub-skills/mm-execute.md`
- `[skill-dir]/sub-skills/mm-review.md`
- `[skill-dir]/sub-skills/mm-track.md`
- `[skill-dir]/sub-skills/mm-templates.md`

External delegated skills (discovered via skill discovery) are activated via the
Skill tool using the skill's registered name.
...
### Extended Thinking Offer Protocol

Pause and ask *"This step would benefit from extended thinking (uses extended
thinking mode for deeper reasoning). Use it?"* when any of the following
...
1. Read the available-skills list from the Claude Code system prompt
...
4. Offer extended thinking for borderline confidence matches
...
Read the appropriate sub-skill file before activating it (paths use `[skill-dir]`
