...
execution. This protocol is Claude Code / Claude Desktop-native.

## Discovery Algorithm

### Step 1: Read Available Skills

Read the available-skills list from Claude's context. In Claude Code, this
list is injected via system-reminder at session start and reflects what is
...
If the top match is Medium confidence, offer extended thinking before presenting:

> "Skill matching is uncertain here. Use extended thinking for a more thorough
> evaluation? (uses extended thinking mode)"
...
1. Record a `uses_skill` self-edge on the Task node using the skill's
   registered name as it appears in the available-skills list:
...
7. Checkpoint to mnem

## Caching

The available-skills list is already in Claude's context for the session —
